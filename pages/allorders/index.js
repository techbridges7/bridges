...
function AllCases() {
  ...
  const updateStatusToDelivered = async (id) => {
    const deliveryRef = doc(db, 'Deliveries', id);
    try {
      await updateDoc(deliveryRef, { status: 'Delivered' });
    } catch (error) {
  ...
  >
    {item.status}
        ...
        <button 
          ...
          onClick={() => updateStatusToDelivered(item.id)}
        >
          Mark as Delivered
        </button>
      </td>
...
export default WithProtected(AllCases)
